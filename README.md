...
> The entire Doodles NFT universe in a single git repository.

doodlebase is the entire Doodles NFT Universe stored in a single Git repository. It's made up of:
...
- Try the Demo: https://doodlebase.mixtape.network
...
With doodlebase, you can simply download the entire NFT universe through a single git clone and work with it 100% locally and offline. Use it in your app as often as you want, for free, forever!.
...
doodlebase indexes all properties of the NFT metadata inside a table named **metadata**, inside the [mixtape.db](mixtape.db) DB, and makes them queryable. Here are the attributes indexed:
...
doodlebase also lets you access all the associated IPFS files WITHOUT running an IPFS node!

Simply clone this git repository and it contains all the IPFS files for the doodlebase NFT Universe! You can check out all the files under [ipfs](ipfs).

## 3. WEB APP

doodlebase comes with a built-in query engine web app at [index.html](index.html)
...
You can try the web app at https://doodlebase.mixtape.network
...
```
git clone https://github.com/mixtape-network/doodlebase.git
```

## 2. Try the query web app

You can now run the entire doodlebase query engine web app on your local machine. **No network connection required since everything is on your machine**.

Simply go into the `doodlebase` folder you just downloaded, and run:
...
1. **Playing with the files:** Go into the doodlebase folder you just cloned, and play with all the files under the `ipfs` folder.
...
The built-in website lets you quickly query the DB at https://doodlebase.mixtape.network WITHOUT a server. Everything is loaded from the static page, **including the [mixtape.db](mixtape.db) DB.**
